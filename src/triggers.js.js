...
/** Backfill Master Source from Roster by Email (Group -> Source) **/
function backfillMasterSourceFromRoster_(quiet){
  const ss = SpreadsheetApp.getActive();
  const roster = ss.getSheetByName(CFG.SHEET_ROSTER);
  if (!roster) return;
  const rMap = mapRosterHeaders_(roster);
  if (!rMap || rMap.group == null) return; // Need Roster Group column
  ...
  const mHdr = normalizeHeaderRow_(mVals[0]);
  const mMap = mapHeaders_(mHdr); // Need Master mapping
  // Check Master columns: Source, PTIN, Email
  if (mMap.ptin == null || mMap.email == null || mMap.source == null) return;
  ...
  for (let i=1;i<rVals.length;i++){
    ...
    // Group must have a value to push
    if (!group) continue; 
  ...
  // 2. Iterate Master and backfill Source if currently blank
  for (let i=0;i<body.length;i++){
    const row = body[i];
    const email = String(row[mMap.email]||'').toLowerCase().trim();
    const ptin  = formatPtinP0_(row[mMap.ptin]||'');
    const source  = String(row[mMap.source]||'').trim();
    
    const key = email || ptin;
    
    if (key && !source && emailOrPtinToGroup.has(key)) {
      // Master Source is blank, and we have a Group value from Roster
      row[mMap.source] = emailOrPtinToGroup.get(key);
      changes++;
    }
  }
  ...
  if (changes) {
    ...
    if (!quiet) toast_(`Master Source backfilled from Roster Group: ${changes} row(s) updated.`);
  }
}
